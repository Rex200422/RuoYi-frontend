    ...
    <div class="page-section">
      <div class="top-bar">
        ...
        <div class="filter-pills" style="margin-left:8px;">
          <span class="filter-pill" :class="{ active: crawlLogSelectedSite === '' }" @click="handleCrawlLogFilter('')">全部站点</span>
          <span v-for="p in allPlatforms" :key="p.platform_name" class="filter-pill" :class="{ active: crawlLogSelectedSite === p.platform_name }" @click="handleCrawlLogFilter(p.platform_name)">{{ p.icon }} {{ p.display_name || p.platform_name }}</span>
        </div>
        <button class="btn btn-outline btn-sm" @click="loadCrawlLogs">🔄 刷新</button>
      </div>
      <div class="card">
        <div class="card-header"><span class="card-title"><span class="dot dot-green"></span>爬取任务日志</span><span class="count-indicator">共 {{ crawlLogTotal }} 条{{ crawlLogSelectedSite ? ' (已筛选)' : '' }}</span></div>
        <div class="table-wrapper">
          <table v-if="crawlLogs.length > 0">
          ...
          <div v-else class="empty-state"><div class="icon">📋</div><p>暂无爬取日志数据</p></div>
        </div>
...
const crawlLogSelectedSite = ref(''); const allPlatforms = ref([])
function handleCrawlLogFilter(f) {
  crawlLogSelectedSite.value = f
  crawlLogQuery.pageNum = 1
  loadCrawlLogs()
}
...
async function loadCrawlLogs() {
  ...
  try {
    const p = { pageNum: crawlLogQuery.pageNum, pageSize: crawlLogQuery.pageSize }
    if (crawlLogSearch.value) p.searchValue = crawlLogSearch.value
    if (crawlLogSelectedSite.value) p.siteName = crawlLogSelectedSite.value
...
onMounted(() => { loadPlatforms(); loadCrawlLogs() })
</script>
